...
All inferred state lives under `.harness/knowledge/state/`. These are generated artifacts — safe to delete and regenerate via `bootstrap.py scan`.
...
Ambiguities and low-confidence items go to `.harness/knowledge/docs/quality/findings/open-items.yaml` rather than being guessed. Findings include type, detail, evidence, and confidence.
