...
Durable development contracts live in `.harness/workspace/contracts/YYYY-MM-DD/` with outstanding contracts tracked in `.harness/workspace/contracts/INDEX.md` and `.harness/workspace/contracts/INDEX.json`. The pipeline runner supports `contract-add`, `contract-update`, and `rebuild-contract-index` subcommands.
