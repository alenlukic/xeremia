...
RUN_LEDGER(required=true, source=RUN_LEDGER.md, publish=.harness/history/ledgers/)
...
6. Separate persistent knowledge from ephemeral artifacts.
   Design docs and research belong in .harness/knowledge/docs/. Ephemeral run artifacts belong in .harness/history/runs/. Durable curated learnings belong in .harness/history/ledgers/.
...
## Additional Operating Defaults

- Prefer follow-on runs over same-run scope explosions.
- Treat breaker findings as falsification evidence, not just review commentary.
- Keep long-term memory compact, indexed, and reconstructible.
- Use token-aware context packaging: task + plan + diff + ledger before broader file dumps.
- Keep schedules deterministic and human-reviewable; do not hide autonomous work behind opaque background behavior.
