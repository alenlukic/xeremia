...
python3 .harness/control/bin/bootstrap.py plan

# Apply approved/high-confidence changes
python3 .harness/control/bin/bootstrap.py apply

# Verify everything is consistent
python3 .harness/control/bin/bootstrap.py doctor
```
...
Full repo traversal. Detects manifests, docs, scripts, framework markers, instruction surfaces, and entrypoints. Writes all state files under `.harness/knowledge/state/`.

```bash
python3 .harness/control/bin/bootstrap.py scan
python3 .harness/control/bin/bootstrap.py scan --repo-root /path/to/repo
```

Does NOT modify any user-facing docs. Safe to run at any time.

**Output files:**
- `.harness/knowledge/state/repo-profile.yaml`
- `.harness/knowledge/state/repo-inventory.json`
- `.harness/knowledge/state/module-map.yaml`
- `.harness/knowledge/state/command-registry.json`
- `.harness/knowledge/state/docs-sync-state.json`
- `.harness/knowledge/state/pending-doc-updates.yaml`
- `.harness/knowledge/docs/quality/findings/open-items.yaml`
...
```bash
python3 .harness/control/bin/bootstrap.py sync
```
