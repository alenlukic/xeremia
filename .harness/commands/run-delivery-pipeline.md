...
- if `task_file` is provided, use:
  - `python3 .harness/bin/pipeline.py start --mode delivery --task-file <task_file> [--parent-run ... --source-kind ... --source-artifact ...]`
- otherwise use:
...
- if actionable breaker findings remain, the default next step is:
  ...
  - record linkage with `python3 .harness/bin/pipeline.py record-follow-on ...`
...
- `python3 .harness/bin/pipeline.py publish-ledger --run-dir <run_dir>` when the run produced durable learnings
