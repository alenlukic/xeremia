...
- **Visible UI integrity.** For UI-heavy workflows like the set explorer, the user needs the live app to prove that expected elements render, stay mounted when they should, and avoid runtime console noise; mocked-only confidence is not enough. *(April 2026: shared audition playback now uses a HEAD preflight on `/api/tracks/:id/audio` before media load, which reduces broken-player trust failures when audio is unavailable.)*
- **Drag-and-drop reliability.** Each DnD destination must produce a visible state change. "Mostly works" is not acceptable when some drop targets silently fail. Accidental mutations from plain clicks or stale interaction state erode trust rapidly.
...
2. **Set preparation:** Pick a starting track → find matches → follow transition chains (A→B→C via "Use as source") → build a named set → verify transitions → export to m3u8. *(Server-persisted set workspace shipped April 2026: PostgreSQL-backed sets with pool, tracklist, and visual explorer canvas. Per-track notes on tracklist entries for cue/mix reminders. Shared audition playback now follows the user across Browse, Matches, Pool, Tracklist, and Explorer via one persistent player bar. The Explorer now supports multiple persisted trees within a single set. Pool subgroup curation now uses stable `All` and `Groups` modes, and creating a subgroup keeps the DJ anchored in the current view instead of auto-switching focus. Pool and Tracklist entries support per-entry starring and surface-specific bulk clear. Tracks can be dragged from Browse/Matches into Set/Explorer via stretched dock tabs with hover-to-open. Transition scoring, reorder, and m3u8 export remain part of the workflow. Explorer verification now expects live DOM/render checks for canvas stability, not just unit coverage. April 2026 also delivered a verified two-column Set Mode layout with the tracklist stack on the left (37%), the Explorer canvas on the right (63%), and no extra row above the split. USB/Rekordbox handoff remains deferred.)*
...
4. **Collection audit:** Browse by key/BPM/genre to identify gaps ("I have nothing in 4A around 124 BPM") or oversaturation. *(April 2026: Browse, Matches, and Pool now support multi-sort, and Browse restores prior list position when the user returns from adjacent workflows. The expanded filter tray can stay collapsed while filters remain active, and `Clear Filters` resets the full filter set.)*
