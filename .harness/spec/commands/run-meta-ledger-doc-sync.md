...
Read only published ledgers in `.harness/history/ledgers/` that are pending since the last sync state.

Allowed edits:
- repo docs
- harness docs
- lightweight structural metadata (indexes, catalogs, manifests, navigation docs)
- stable persona guidance under `.harness/workspace/product-feedback/`
...
Delegate to `Meta Ledger Doc Steward`.

## DO

1. Determine ledger set
- inspect `.harness/history/ledgers/DOC_SYNC_STATE.json`
- use `python3 .harness/control/bin/pipeline.py pending-ledgers` unless `since_run_id` overrides the boundary
...
- if `allow_persona_updates=false`, do not edit `.harness/workspace/product-feedback/*`

4. Write sync report
- write `.harness/history/ledgers/DOC_SYNC_REPORT.md` with:
  - ledgers consumed
  - files updated
...
- call `python3 .harness/control/bin/pipeline.py mark-doc-sync --up-to-run <latest_consumed_run_id>`
...
- updated `.harness/history/ledgers/DOC_SYNC_STATE.json`

## ACCEPTANCE

Complete only if:
- the Meta Ledger Doc Steward agent was used
