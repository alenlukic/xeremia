...
into `.harness/control/pipeline.yaml` and cleans up source-project-specific files.
...
Modify only:
- `.harness/control/pipeline.yaml` — replace `true` placeholders with real commands
- `docs/` — stub/remove per MANIFEST `replace` entries
- `.harness/control/rules/30-client-ui.mdc` — remove per MANIFEST if applicable
- `.harness/knowledge/docs/design-docs/` — reset per MANIFEST if applicable

Do not modify:
- `AGENTS.md`, `HUMANS.md`, `CLAUDE.md` — these are correct as-is
- `.harness/control/bin/pipeline.py`, `.harness/spec/agents/`, `.harness/spec/commands/`
- `.harness/knowledge/docs/core-beliefs.md`, `.harness/knowledge/docs/quality/`, `.template/`
...
### 2. Configure .harness/control/pipeline.yaml commands

Read `.harness/control/pipeline.yaml`. For each `commands` entry that still contains `true  # configure:`:
...
Confirm:
- `.harness/control/pipeline.yaml` has real commands (no remaining `true` placeholders)
...
Complete only if:
- `.harness/control/pipeline.yaml` has real commands or the user explicitly accepted leaving some as `true`
