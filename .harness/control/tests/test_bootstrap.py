...
BOOTSTRAP = os.path.join(REPO_ROOT, ".harness", "control", "bin", "bootstrap.py")
...
def test_scan_runs_without_error():
    ...
    state_dir = os.path.join(REPO_ROOT, ".harness", "knowledge", "state")
...
def test_doctor_detects_stale_reference():
    ...
    reg_path = os.path.join(REPO_ROOT, ".harness", "knowledge", "state", "command-registry.json")
...
def test_apply_noop_without_generated_section():
    ...
    pu_path = os.path.join(REPO_ROOT, ".harness", "knowledge", "state", "pending-doc-updates.yaml")
    try:
        import yaml
