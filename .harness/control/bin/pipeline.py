...
CONTROL_DIR = HARNESS / 'control'
SPEC_DIR = HARNESS / 'spec'
INTAKE_DIR = HARNESS / 'intake'
WORKSPACE_DIR = HARNESS / 'workspace'
KNOWLEDGE_DIR = HARNESS / 'knowledge'
HISTORY_DIR = HARNESS / 'history'

RUNS_DIR = HISTORY_DIR / 'runs'
LEDGERS_DIR = HISTORY_DIR / 'ledgers'
CONTRACTS_DIR = WORKSPACE_DIR / 'contracts'
PF_DIR = WORKSPACE_DIR / 'product-feedback'
STATE_MACHINE_FILE = CONTROL_DIR / 'state_machine' / 'STATE_MACHINE.yaml'
SCHEDULES_FILE = CONTROL_DIR / 'schedules' / 'SCHEDULES.yaml'
SCHEDULE_STATE_FILE = CONTROL_DIR / 'schedules' / 'STATE.json'
CONFIG_PATH = CONTROL_DIR / 'pipeline.yaml'
...
def state_machine_render() -> str:
    ...
    for mode, spec in sm.get('modes', {}).items():
        ...
        for st in spec.get('states', []):
            ...
            prev = node
    mermaid = '\n'.join(lines) + '\n'
    write_text(CONTROL_DIR / 'state_machine' / 'STATE_MACHINE.mmd', mermaid)
    return mermaid
