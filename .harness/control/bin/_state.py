"""State layer: read/write for .harness/knowledge/state/ artifacts."""
...
def _state_dir(repo_root: str) -> Path:
    return Path(repo_root) / ".harness" / "knowledge" / "state"
...
def open_items_path(repo_root: str) -> Path:
    return Path(repo_root) / ".harness" / "knowledge" / "docs" / "quality" / "findings" / "open-items.yaml"
