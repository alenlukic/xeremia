...
def _check_state_files_exist(root: Path, report: dict) -> None:
    state_dir = root / ".harness" / "knowledge" / "state"
    expected = [
        "repo-profile.yaml",
        "repo-inventory.json",
        "module-map.yaml",
    ...
    for f in expected:
        ...
        if not fp.exists():
            ...
            report["warnings"].append(f"State file missing: .harness/knowledge/state/{f}")
    report["checks"].append(check)


def _check_command_references(root: Path, report: dict) -> None:
    """Verify commands in registry point to real files/scripts."""
    reg_path = root / ".harness" / "knowledge" / "state" / "command-registry.json"
...
def _check_stale_module_descriptions(root: Path, report: dict) -> None:
    ...
    mm_path = root / ".harness" / "knowledge" / "state" / "module-map.yaml"
