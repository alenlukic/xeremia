...
SKIP_DIRS = {
    ...
    ".harness/history/runs", ".harness/history/pr_descriptions",
}
...
def _infer_modules(inventory: dict[str, Any], repo_root: str) -> list[dict[str, Any]]:
    ...
    if harness_indicators or (root / ".harness").is_dir():
        modules.append({
            ...
            "type": "tooling",
            "confidence": 0.95,
            "evidence": harness_indicators[:3] if harness_indicators else [".harness/control/pipeline.yaml"],
...
def _infer_commands(inventory: dict[str, Any], repo_root: str) -> list[dict[str, Any]]:
    ...
    pipeline_yaml = root / ".harness" / "control" / "pipeline.yaml"
    if pipeline_yaml.exists():
        commands.append({
            "command_id": "pipeline-runner",
            "kind": "build",
            "canonical": "python3 .harness/control/bin/pipeline.py",
            "alternates": [],
            "scope": "repo",
            "confidence": 0.95,
            "evidence": ".harness/control/pipeline.yaml",
            "last_verified": None,
        })

    setup_sh = root / ".harness" / "control" / "bin" / "setup.sh"
    if setup_sh.exists():
        commands.append({
            "command_id": "ide-setup",
            "kind": "local-run",
            "canonical": "bash .harness/control/bin/setup.sh",
            "alternates": [],
            "scope": "repo",
            "confidence": 0.95,
            "evidence": ".harness/control/bin/setup.sh",
            "last_verified": None,
        })

    bootstrap_py = root / ".harness" / "control" / "bin" / "bootstrap.py"
    if bootstrap_py.exists():
        commands.append({
            "command_id": "bootstrap",
            "kind": "local-run",
            "canonical": "python3 .harness/control/bin/bootstrap.py",
            "alternates": [],
            "scope": "repo",
            "confidence": 0.95,
            "evidence": ".harness/control/bin/bootstrap.py",
            "last_verified": None,
        })
