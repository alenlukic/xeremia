...
import { AudioPlayerProvider, useAudioPlayer, _validationCache, normalizeMime, browserCanPlay, aiffToWav, parseAiffHeader, buildWavHeader, canUseStreamingMse, streamAiffAsWav, type AiffMetadata } from './useAudioPlayer';
...
function buildAiffBuffer(
  channels: number, frames: number, bitsPerSample: number, sampleRate: number,
  pcmSamples?: number[],
): ArrayBuffer {
  ...
  const mant = Math.round((sampleRate / Math.pow(2, Math.floor(Math.log2(sampleRate)))) * 0x80000000);
...
describe('AudioPlayerProvider', () => {
  ...
  it('validates audio endpoint with HEAD then streams on play()', async () => {
    ...
    expect(window.fetch).toHaveBeenCalledWith('/api/tracks/1/audio', { method: 'HEAD' });
  ...
  it('shows error on fetch failure', async () => {
    ...
    expect(screen.getByTestId('error').textContent).toBe('Track audio not found');
    expect(screen.getByTestId('playing').textContent).toBe('false');
  ...
  it('shows format error for 415 status', async () => {
    ...
    expect(screen.getByTestId('error').textContent).toBe('Audio format not supported by server');
    expect(screen.getByTestId('loading').textContent).toBe('false');
  });
  ...
  it('rejects truly unknown format when canPlayType returns empty', async () => {
    ...
    expect(screen.getByTestId('error').textContent).toBe('Audio format not supported by this browser');
    expect(screen.getByTestId('loading').textContent).toBe('false');
  ...
  it('maps audio element error code to app-level message', async () => {
    ...
    if (errorHandler) {
      mockAudioElement.error = { code: 4, message: 'Failed to load because no supported source was found.' };
      await act(async () => { errorHandler(); });
    }

    expect(screen.getByTestId('error').textContent).toBe('Audio format not supported');
  });

  it('maps play() NotSupportedError to app-level message', async () => {
    mockPlay.mockRejectedValueOnce(new DOMException('The element has no supported sources.', 'NotSupportedError'));
...
describe('Track switching cancellation safety', () => {
  ...
  it('stale fetch resolution from track A cannot overwrite track B', async () => {
    ...
    await act(async () => {
      resolveTrackB({ ok: true, headers: new Headers({ 'content-type': 'audio/mpeg' }) } as Response);
    });
    expect(screen.getByTestId('track-title').textContent).toBe('Second Track');
    expect(mockAudioElement.src).toBe('/api/tracks/2/audio');
    ...
    await act(async () => {
      resolveTrackA({ ok: true, headers: new Headers({ 'content-type': 'audio/mpeg' }) } as Response);
    });
    expect(screen.getByTestId('track-title').textContent).toBe('Second Track');
    expect(mockAudioElement.src).toBe('/api/tracks/2/audio');
  });
  ...
  it('stale error from track A does not set error state when track B is active', async () => {
    ...
    vi.spyOn(window, 'fetch').mockImplementation((url) => {
      ...
      return Promise.resolve({ ok: true, headers: new Headers({ 'content-type': 'audio/mpeg' }) } as Response);
    });
...
describe('MIME normalization helpers', () => {
  ...
  it('browserCanPlay checks normalized type', () => {
    ...
    (el.canPlayType as ReturnType<typeof vi.fn>).mockImplementation((mime: string) =>
...
describe('Content-Type normalization in playback', () => {
  ...
  it('plays AIFF track via client-side transcode when browser only supports wav', async () => {
    (mockAudioElement.canPlayType as ReturnType<typeof vi.fn>).mockImplementation(
      (mime: string) => (mime === 'audio/wav' ? 'probably' : ''),
    );
  ...
  it('plays AIFF track when browser supports x-aiff alias', async () => {
    (mockAudioElement.canPlayType as ReturnType<typeof vi.fn>).mockImplementation(
      (mime: string) => (mime === 'audio/x-aiff' ? 'maybe' : mime === 'audio/mpeg' ? 'probably' : ''),
    );
    vi.spyOn(window, 'fetch').mockResolvedValue({
      ok: true,
      headers: new Headers({ 'content-type': 'audio/aiff' }),
...
function makeCommChunkData(ch: number, frames: number, bits: number, rate: number): Uint8Array {
  ...
  const mant = Math.round((rate / Math.pow(2, Math.floor(Math.log2(rate)))) * 0x80000000);
...
function makeAifcCommChunkData(
  ch: number, frames: number, bits: number, rate: number, compType: string,
): Uint8Array {
  ...
  v.setUint32(10, mant, false);
  v.setUint32(14, 0, false);
  for (let i = 0; i < 4; i++) v.setUint8(18 + i, compType.charCodeAt(i));
  v.setUint8(22, 1);
  v.setUint8(23, 0);
  return new Uint8Array(buf);
}

function makeSsndChunkData(frames: number, channels: number, bytesPerSample: number): Uint8Array {
...
describe('parseAiffHeader', () => {
  ...
  it('throws on AIFC compressed format', () => {
    ...
    expect(() => parseAiffHeader(buf)).toThrow('Unsupported AIFF-C compression: ima4');
  });
...
describe('buildWavHeader', () => {
  it('produces a 44-byte WAV header', () => {
    const meta: AiffMetadata = {
      channels: 2, frames: 100, bitsPerSample: 16,
      sampleRate: 44100, ssndDataOffset: 0, ssndDataSize: 400,
    };
  ...
  it('throws on Uint32 overflow from crafted metadata', () => {
    const meta: AiffMetadata = {
      channels: 2, frames: 0x80000000, bitsPerSample: 16,
      sampleRate: 44100, ssndDataOffset: 0, ssndDataSize: 400,
    };
...
describe('Streaming AIFF via MSE', () => {
  ...
  class MockMediaSource extends EventTarget {
    readyState = 'closed';
    addSourceBuffer(_mime: string) { return new MockSourceBuffer(); }
    endOfStream(_r?: string) { this.readyState = 'ended'; }
    static isTypeSupported(m: string) { return m.startsWith('audio/wav'); }
    constructor() {
      super();
      queueMicrotask(() => {
  ...
  it('seek-ahead while streaming does not crash', async () => {
    ...
    await act(async () => {
      const player = screen.getByTestId('play-btn');
...
describe('streamAiffAsWav direct', () => {
  it('throws when fetch returns non-ok status', async () => {
    ...
    const el = mockAudioElement as unknown as HTMLAudioElement;
    await expect(streamAiffAsWav('/audio', el, () => false)).rejects.toThrow('Audio unavailable (500)');
  });
  ...
  it('throws when response has no body', async () => {
    ...
    const el = mockAudioElement as unknown as HTMLAudioElement;
    await expect(streamAiffAsWav('/audio', el, () => false)).rejects.toThrow('Streaming not supported');
  });
  ...
  it('returns immediately when cancelled during header parse', async () => {
    ...
    vi.spyOn(window, 'fetch').mockResolvedValue({
      ...
      body: {
        getReader: () => ({
          read: () => Promise.resolve({ done: false, value: new Uint8Array(4) }),
          cancel: readerCancel,
        }),
      },
    } as unknown as Response);
  ...
  it('throws descriptive error when stream ends before header complete', async () => {
    vi.spyOn(window, 'fetch').mockResolvedValue({
      ...
      body: {
        getReader: () => ({
          read: vi.fn()
            .mockResolvedValueOnce({ done: false, value: new Uint8Array([0x46, 0x4f, 0x52, 0x4d]) })
    ...
    const el = mockAudioElement as unknown as HTMLAudioElement;
    await expect(streamAiffAsWav('/audio', el, () => false))
      .rejects.toThrow('AIFF file truncated: stream ended before header complete');
  });
});
...
describe('Streaming MSE fallback to full download', () => {
  it('falls back to full-download when streaming throws', async () => {
    ...
    (globalThis as Record<string, unknown>).MediaSource = class extends EventTarget {
      readyState = 'open';
      addSourceBuffer() { return new EventTarget(); }
      endOfStream() {}
      static isTypeSupported(m: string) { return m.startsWith('audio/wav'); }
    };
