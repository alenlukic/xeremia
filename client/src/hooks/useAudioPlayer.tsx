...
function endianSwapPcm(
  input: Uint8Array,
  bytesPerInputSample: number,
): Uint8Array {
  ...
  for (let i = 0; i < sampleCount; i++) {
    outView.setInt16(
      i * 2,
      inView.getInt16(i * bytesPerInputSample, false),
      true,
    )
  }
...
export function canUseStreamingMse(): boolean {
  ...
  return (
    MediaSource.isTypeSupported('audio/wav; codecs="1"') ||
...
export async function streamAiffAsWav(
  ...
  if (accumulated.length > ssndDataOffset) {
    ...
    const usable =
      Math.floor(available.length / bytesPerInputSample) * bytesPerInputSample
    if (usable > 0) {
      await appendToSourceBuffer(
        sourceBuffer,
        endianSwapPcm(available.subarray(0, usable), bytesPerInputSample),
      )
  ...
  const background = async () => {
    try {
      while (pcmBytesProcessed < totalPcmBytes) {
        ...
        const usable =
          Math.floor(chunk.length / bytesPerInputSample) * bytesPerInputSample
        ...
        if (usable > 0) {
          ...
          await appendToSourceBuffer(
            sourceBuffer,
            endianSwapPcm(chunk.subarray(0, usable), bytesPerInputSample),
          )
          pcmBytesProcessed += usable
        }
      }
  ...
  if (err instanceof DOMException) {
    switch (err.name) {
      ...
      case 'NotAllowedError':
        return 'Playback blocked by browser'
      case 'AbortError':
...
function readStoredVolume(): number {
  try {
    ...
    return !Number.isFinite(val) || val < 0 || val > 1 ? 0.8 : val
  } catch {
    return 0.8
  }
}
...
export function AudioPlayerProvider({ children }: { children: ReactNode }) {
          ...
          try {
            ...
            if (!audioResp.ok) {
              setState((s) => ({
                ...s,
                loading: false,
                error: `Audio unavailable (${audioResp.status})`,
              }))
              return
            }
  ...
  const value = useMemo<AudioPlayerContextValue>(
    ...
    [state, play, pause, resume, togglePlayPause, seek, setVolume, stop],
  )
