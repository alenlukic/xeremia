...
export async function updateSet(setId: number, name: string): Promise<SetSummary> {
  const res = await fetch(`/api/sets/${setId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name }),
...
export async function poolMoveToTracklist(setId: number, trackId: number): Promise<void> {
  const res = await fetch(`/api/sets/${setId}/pool/move-to-tracklist`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ track_id: trackId }),
  });
  if (!res.ok) throw new Error(`Pool move to tracklist failed: ${res.status}`);
}

export async function tracklistAdd(setId: number, trackId: number): Promise<void> {
...
export async function updateTracklistNote(
  setId: number, trackId: number, note: string,
): Promise<void> {
...
export async function explorerDeleteNode(
  ...
  if (!res.ok) throw new Error(`Explorer delete node failed: ${res.status}`);
}
...
export async function explorerAddEdge(
  ...
  const res = await fetch(`/api/sets/${setId}/explorer/edges`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ parent_node_id: parentNodeId, child_node_id: childNodeId }),
  });
...
export async function explorerNodeToTracklist(
  ...
  if (!res.ok) throw new Error(`Explorer node to tracklist failed: ${res.status}`);
}

export async function explorerEdgeScores(
  setId: number, pairs: [number, number][],
): Promise<{ scores: (number | null)[] }> {
