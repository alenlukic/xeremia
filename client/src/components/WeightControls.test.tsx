...
describe('Weight save status indicators', () => {
  ...
  it('shows warning message when weights sum is invalid', () => {
    ...
    expect(screen.getByText('Weights sum to 110; target is 100')).toBeInTheDocument();
  });
  ...
  it('prioritizes error over warning and saving states', () => {
    ...
    expect(screen.queryByText('Weights sum to 90; target is 100')).not.toBeInTheDocument();
  });
  ...
  it('shows "Saving…" alongside warning during invalid-sum save', () => {
    ...
    expect(screen.getByText('Saving…')).toBeInTheDocument();
    expect(screen.getByText('Weights sum to 90; target is 100')).toBeInTheDocument();
  });
  ...
  it('shows "Saved" alongside warning after successful invalid-sum save', () => {
    ...
    expect(screen.getByText('Saved')).toBeInTheDocument();
    expect(screen.getByText('Weights sum to 80; target is 100')).toBeInTheDocument();
  });
