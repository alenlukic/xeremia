...
describe('SortTierBar', () => {
  ...
  it('toggles direction on dir button click', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /toggle title direction/i }),
    )
  ...
  it('reorders tiers via move-down button', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /move title sort down/i }),
    )
    expect(onChange).toHaveBeenCalledWith([
      { id: 'bpm', desc: true },
      { id: 'title', desc: false },
  ...
  it('does not show move-up on first tier or move-down on last tier', () => {
    ...
    expect(
      screen.queryByRole('button', { name: /move title sort up/i }),
    ).toBeNull()
    expect(
      screen.queryByRole('button', { name: /move bpm sort down/i }),
    ).toBeNull()
  })
})
