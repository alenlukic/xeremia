...
export function SortTierBar({ sorting, columns, onSortingChange }: SortTierBarProps) {
  const [menuOpen, setMenuOpen] = useState(false);
  const wrapperRef = useRef<HTMLSpanElement>(null);

  const availableColumns = columns.filter(c => !sorting.some(s => s.id === c.id));
  ...
  const addTier = useCallback((colId: string) => {
  ...
  const toggleDirection = useCallback((idx: number) => {
    const next = [...sorting];
    next[idx] = { ...next[idx], desc: !next[idx].desc };
    onSortingChange(next);
  }, [sorting, onSortingChange]);

  const moveTierUp = useCallback((idx: number) => {
    if (idx <= 0) return;
    const next = [...sorting];
  ...
  const moveTierDown = useCallback((idx: number) => {
  ...
  if (sorting.length === 0 && availableColumns.length === 0) return null;
