...
type FieldDef = {
  label: string
  getValue: (t: MatchDetailTrackInfo) => unknown
}
...
export function MatchDetail({
  ...
  useEffect(() => {
    ...
    fetchMatchDetail(sourceTrack.id, match.candidate_id)
      .then((result) => {
        if (traitMap.size > 0) {
          result.on_deck.traits =
            traitMap.get(result.on_deck.id) ?? result.on_deck.traits
          result.candidate.traits =
            traitMap.get(result.candidate.id) ?? result.candidate.traits
        }
    ...
    <div className="match-detail">
      ...
      <div className="detail-header">
        <h2 className="detail-title">
          Match Detail —{' '}
          <span className="mono">
            {formatOverallScore(detail.overall_score)}
          </span>
        </h2>
            ...
            <div key={track.id} className="detail-track-card">
              ...
              <div className="detail-card-fields">
                {FIXED_ROWS.map((row, ri) => (
                  <div
                    key={ri}
                    className={`detail-row detail-row--${row.length}`}
                  >
                    {row.map((field, fi) =>
                      field ? (
