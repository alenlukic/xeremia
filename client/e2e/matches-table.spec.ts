...
const MOCK_MATCHES = [
  makeMatch(2, 'same_key'),
  makeMatch(3, 'same_key'),
  makeMatch(4, 'same_key'),
  makeMatch(5, 'higher_key'),
  makeMatch(6, 'lower_key'),
]
...
async function mockAPIs(page: Page) {
  await page.route('/api/tracks**', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify([MOCK_TRACK]),
    }),
  )
  await page.route('/api/track-traits', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: '[]',
  ...
  await page.route('/api/weights', (route) => {
    if (route.request().method() === 'PUT') {
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(MOCK_WEIGHTS),
      })
    }
    return route.fulfill({
      status: 200,
  ...
  await page.route('/api/weights/defaults', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(MOCK_WEIGHTS.raw_weights),
    }),
  )
  ...
  await page.route('/api/tracks/1/matches', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('/api/admin/cache-stats', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        misses: 3,
        hit_rate: 0.4,
        hit_rate_numerator: 2,
        hit_rate_denominator: 5,
        hit_rate_basis: 'all',
        key_distribution: [],
        bpm_distribution: [],
        recent_entries: [],
...
async function selectTrackAndShowMatches(page: Page) {
  ...
  await page
    .locator('.matches-table thead th')
    .first()
...
test.describe('Matches table interactions', () => {
  ...
  test('Track column resize changes header width', async ({ page }) => {
    ...
    const widthBefore = await trackHeader.evaluate(
    ...
    const widthAfter = await trackHeader.evaluate(
      (el) => el.getBoundingClientRect().width,
    )
  ...
  test('header and body cells remain aligned after Track resize', async ({
    page,
  }) => {
    const trackHeader = page.locator('.matches-table thead th').nth(1)
    const resizer = trackHeader.locator('.col-resizer')
  ...
  test('overflow top-scrollbar syncs with table wrapper', async ({
    browser,
  }) => {
    ...
    await topScroll.evaluate((el) => {
      ;(el as HTMLElement).style.minHeight = '12px'
    })
