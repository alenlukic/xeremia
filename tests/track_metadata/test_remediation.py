...
def test_stage_classify_uses_exact_mission_critical_set():
    ...
    stage_classify(
        result, PipelineContext(hydrator=MagicMock(), run_report=RunReport())
    )

    assert set(MISSION_CRITICAL_FIELDS) == {
        "key",
        "bpm",
        "camelot_code",
        "artist",
...
def test_stage_persist_routes_to_remediation_when_critical_missing(
    ...
    result = TrackResult(
        source=Path("track.mp3"), metadata=SimpleMetadata(title="T", artist="A")
    )
